...
.app-menu-open .app-menu {
	...
	transition: transform 300ms linear;
      ...
      <p className="content">
        To make animations smoother and faster, we can tell the browser to use
        the GPU instead of the CPU. The GPU is much better at handling visual
        effects, so animations like movement and fading look more fluid when
        it’s in charge. The modern way to do this is with the CSS property
        will-change. For example, adding will-change: transform to an element
        lets the browser know ahead of time that you'll animate its position, so
        it can prepare and optimize it for better performance. Before
        will-change was available, developers used a trick like transform:
        translateZ(0) or translate3d(0, 0, 0) to force the browser to use the
        GPU. These still work, but will-change is cleaner and more reliable
        today. In short, if you're animating transform or opacity, use
        will-change to get smoother animations by offloading the work to the
        GPU.
      </p>
      <pre>
        <code>
